...
if(isset($_POST['hSubmit'])){ 
    ...
    if(empty($hireName) || empty($hireEmail) || empty($hireContact) || empty($hireSite) || empty($hirePrice) || empty($hireMessage)){
        header("Location: ../hire.php?error=emptyforms"."&hname=".$hireName."&hemail=".$hireEmail.
    ...
    elseif(!filter_var($hireEmail, FILTER_VALIDATE_EMAIL) && !filter_var($hireSite, FILTER_VALIDATE_URL)){
        header("Location: ../hire.php?error=invalidmailsite&hemail=".$hireEmail."&hproposal=".$hireMessage); 
        exit(); 
    }  
    // checking for invalid email
    elseif(!filter_var($hireEmail, FILTER_VALIDATE_EMAIL)){
        header("Location: ../hire.php?error=invalidmail&hemail=".$hireEmail); 
        exit(); 
    }  
    //checking for an invalid website
    elseif(!filter_var($hireSite, FILTER_VALIDATE_URL)){
        header("Location: ../hire.php?error=invalidsite&hsite=".$hireSite); 
        exit();
    }  
    ...
    //insert data into database using prepared statements 
        ...
        if(!mysqli_stmt_prepare($stmt, $sql)){
            header("Location: ../hire.php?error=sqlerror"); 
            exit(); 
        }  else{
            ...
            header("Location: ../hire.php?contact=success"); 
            exit(); 
        }
    } 
